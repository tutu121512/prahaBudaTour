...
<html>
	...
	$('#add').on("click", function() {
		if(count < 10) {
			...
			Materialize.toast('이미지 파일은 1개까지 올릴 수 있습니다.', 3000, 'rounded');
		...
		$(".deleteBtn").click(function(){
			...
			if(result) { 
				alert("삭제되었습니다.");
				location.href="/controller/Complete/praha/adminDelete?boardNo="+$(this).attr("id")+"&page="+$(this).attr("name");
			}
		});
		...
		$(".updateBtn").click(function(){
			...
			var location = 'P';
			$("#ReserveInsert").attr("action","/controller/admin/adminUpdate");
	...
	<main>
	  ...
	  <table style="margin-top:30px"><tr><td>
			<h2 style="padding-top:10px;margin:0px;padding-left:40px;">예약확정(기타)</h2>
	  </td></tr></table>
		<form id="reserveInsert" action="/controller/admin/praha/AdminInsert" method="post" enctype="multipart/form-data" >
	...
	<ul class="pagination" style="float:left;margin-top:0px;padding-left:35%;">
			<li class="page-item">
				<a href="/controller/admin/adminReserveComplete?page=<%=pVO.getPreviPage()%>">
			    <i class="material-icons">chevron_left</i>
			     </a>
			   </li>
			   <%for(int i=pVO.getStartPage(); i <= pVO.getEndPage() ; i++) {%>
			   <li class="waves-effect" id="page<%=i%>"><a class="page-link" href="/controller/admin/adminReserveComplete?page=<%=i%>"><%=i %></a></li>
			   <%} %>
			   <li class="page-item">
				<a href="/controller/admin/adminReserveComplete?page=<%=pVO.getNextPage()%>">
